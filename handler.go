...
func (bs *BearerServer) TokenEndpoint(w http.ResponseWriter, r *http.Request) {
	...
	resp, returncode, err := bs.GenerateIdTokenResponse("RS256", grant_type, refresh_token, scope, code, redirect_uri, at, w, r)
...
func (bs *BearerServer) GetRedirect(w http.ResponseWriter, r *http.Request) {
	...
	_, err = bs.verifier.SessionSave(w, r, "user_session", username)
	if err != nil {
		log.Err(err)
	}
